...
export default async function AdminPage({
  ...
  const totalRevenue =
    allOrders?.reduce((sum, order) => sum + Number(order.price_at_sale), 0) || 0
  const pendingPayments =
    allOrders
      ?.filter((o) => !o.payment_completed)
      .reduce((sum, order) => sum + Number(order.price_at_sale), 0) || 0
  const pendingCargoCount =
    allOrders?.filter((o) => o.payment_completed && !o.cargo_sent).length || 0
  const totalSalesCount = allOrders?.length || 0

  // Filtrelenmiş Siparişleri Çekelim
  let query = supabase
    .from('orders')
